...
- User-contributed plugins (`Tools > Reload Plugins`): `.rhai` scripts — the [Rhai](https://rhai.rs) embedded scripting language, sandboxed by omission (no file/network/process APIs are ever exposed to a script, only three host functions: read/replace the open document's text and set the status message) — that can register custom `:` commands and an `on_save` hook that transforms a document's text before it's written to disk. Loaded from two places: a global, always-on directory (`tachylite/plugins/*.rhai` in the platform config directory) and a project's own `.tachylite/plugins/*.rhai`, which only loads once that project explicitly turns it on (`Tools > Enable Project Plugins`) — since a project's plugin folder could otherwise arrive via a shared/pulled git repo and run unreviewed code the moment it's opened
